...
public class Percolation {
    ...
    public Percolation(int n) {
        ...
        N = n;
        siteCount = n * n;
        openState = new boolean[siteCount + 2];
        fullState = new boolean[N];
    ...
    public boolean isFull(int row, int col) {
        ...
        if (isOpen(row, col)){
            if (row == N){
                fullState[col - 1] = backWash.connected(xyTo1D(row, col), 0);
                if (fullState[col - 1]) {
                    percolateState = true;
                }
                return fullState[col -1];
            } else return backWash.connected(xyTo1D(row, col), 0);
        }
        return false;
    }
